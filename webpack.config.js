...
const addons = (/* string | string[] */ addonsArg) => {
  ...
  return addons.map(addonName =>
    require(`./build-utils/addons/webpack.${addonName || 'bundleanalyzer'}.js`)
  );
};
...
module.exports = env => {
  ...
  // or production
  // console.log(env.env); => dev
  const envConfig = require(`./build-utils/webpack.${env && env.env && `${env.env}` !== 'undefined' ? env.env : 'dev'}.js`);
